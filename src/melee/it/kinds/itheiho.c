...
#ifdef MELEE_NATIVE
#define HEIHO_FLOAT(attrs, index) ((attrs)->values[(index) - 1])
#else
typedef f32 itHeihoAttributes;
#define HEIHO_FLOAT(attrs, index) ((attrs)[index])
#endif
...
void itHeiho_UnkMotion1_Phys(Item_GObj* gobj)
{
    ...
    PAD_STACK(8);
    ip->x40_vel.x =
        ip->facing_dir * HEIHO_FLOAT(attr, ip->xDD4_itemVar.heiho.x21 + 1);
...
static void it_802D8EC8_inline(Item_GObj* gobj)
{
    ...
    ip->x40_vel.x += HEIHO_FLOAT(attr, 5) * rand;
    ip->x40_vel.y = 2.0F;
    ip->x40_vel.z = 1.5f;
    if (ip->facing_dir == -1.0F) {
        HSD_JObjSetRotationY(HSD_GObjGetHSDObj(gobj), 3.1415927F);
...
void itHeiho_UnkMotion4_Phys(Item_GObj* gobj)
{
    ...
    itHeihoAttributes* attr = ip->xC4_article_data->x4_specialAttributes;
    PAD_STACK(8);
    ip->x40_vel.x = 1.5F * (ip->facing_dir *
                            HEIHO_FLOAT(attr, ip->xDD4_itemVar.heiho.x21 + 1));
    if (ip->xDD4_itemVar.heiho.x2C > 960) {
        ip->x40_vel.y = -((-0.04f * ABS(ip->x40_vel.x)) - ip->x40_vel.y);
    }
    if (ip->xDD4_itemVar.heiho.x24 != 0) {
