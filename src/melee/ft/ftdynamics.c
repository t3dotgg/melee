...
#ifdef MELEE_NATIVE
typedef s32 FtDynamicBoneLimit;
#else
typedef FigaTree* FtDynamicBoneLimit;
#endif
...
void ftCo_8009E4A8(Fighter* fp)
{
    ...
    if (fp->x594_b4) {
        FtDynamicBoneLimit* tree =
            fp->ft_data->x2C->x10[fp->x28[fp->anim_id][1]];
        if (tree != NULL) {
            for (i = 0; i < fp->dynamics_num; i++) {
