...
#ifdef MELEE_NATIVE
/* Native pointers use host width, so these globals are separate objects.
 * Keep direct references instead of treating the original 32-bit aggregate as
 * a packed struct. */
#define CSS_ALL_ICONS icons
#define CSS_ALL_DOORS mnCharSel_803F0DFC
#define CSS_ALL_TAGS mnCharSel_803F0E8C
...
void mnCharSel_8025FB50(u8 door, s32 arg1)
{
    ...
    if (mnCharSel_804D6CF5 == 1) {
        lb_80011E24(mnCharSel_804D6CC0, &icon_jobj,
                    CSS_ALL_ICONS[icon_idx].joint_id_1p, -1);
    } else {
        lb_80011E24(mnCharSel_804D6CC0, &icon_jobj,
                    CSS_ALL_ICONS[icon_idx].joint_id_vs, -1);
    }
    HSD_ForeachAnim(icon_jobj, JOBJ_TYPE, TOBJ_MASK, HSD_AObjReqAnim,
                    AOBJ_ARG_AF, 10.0);

    CSS_ALL_ICONS[icon_idx].anim_timer = 0xC;
...
s32 mnCharSel_8025FDEC(u8 door)
{
    ...
    if (c_kind < CKIND_PLAYABLE_COUNT) {
        if (c_kind !=
            CSS_ALL_ICONS[mnCharSel_803F0DFC.doors[door].sel_icon].char_kind)
        {
            mnCharSel_803F0DFC.doors[door].costume = 0;
        }

        {
            CSSIcon* icon = CSS_ALL_ICONS;
...
void mnCharSel_CursorThink(HSD_GObj* gobj)
{
    ...
    if (mnCharSel_804D6CF6 == 0) {
        ...
        if (mnCharSel_804D6CF5 == 1) {
            ...
            if (HSD_PadCopyStatus[cursor->x4].err != 0) {
                if (cursor->x5 != 3) {
                    ...
                    if (CSS_ALL_DOORS.doors[cursor->x4].p_kind == 0) {
                        CSS_ALL_DOORS.doors[cursor->x4].p_kind = 1;
                        mnCharSel_804D6CB0->vs.start.players[cursor->x4]
                            .slot_type = 1;
                        mnCharSel_804D6CB0->vs.start.players[cursor->x4]
                            .nametag = 0x78;
                        CSS_ALL_TAGS[cursor->x4].data->use_tag = 0;
                    }
        ...
        CSS_ALL_TAGS[cursor->x4].data->scroll_amt = 0.0f;

        {
            current_tag = CSS_ALL_TAGS[cursor->x4].data;
            if (current_tag->state != 0) {
                if (mnCharSel_804D6CF5 == 1) {
                    lb_80011E24(mnCharSel_804D6CC0, &sp98,
                                CSS_ALL_MISC.name_list_joint, -1);
                } else {
                    lb_80011E24(mnCharSel_804D6CC0, &sp98,
                                CSS_ALL_TAGS[cursor->x4].list_joint, -1);
                }
                lb_8000B1CC(sp98, NULL, &sp88);

                if ((0.8f + sp88.y) < cursor->x10) {
                    CSS_ALL_TAGS[cursor->x4].data->scroll_amt =
        ...
        {
            u8 port = cursor->x4;
            CSSTagData* tag = CSS_ALL_TAGS[port].data;
            if (tag->state == 0) {
                if (cursor->x5 == 1) {
                    door = cursor->x6;
                    switch (door) {
                    ...
                    case 3:
                        if (cursor->x10 < 0.2f) {
                            if ((s32) port != (s32) door ||
                                ((s32) port == (s32) door &&
                                 CSS_ALL_DOORS.doors[door].p_kind == 3))
                            {
...

                            {
                                            ...
                                            cursor->x5 = 2;
                                            {
                                                u8 sel =
                                                    CSS_ALL_DOORS
                                                        ...
                                                        .sel_icon;
                                                lbAudioAx_80023870(
                                                    CSS_ALL_ICONS[sel].sfx,
                                                    0x7F, 0x40, sel + 0x8A);
                                                gm_80168C5C(
                                                    (u32) CSS_ALL_ICONS[sel]
                                                        .char_kind);
                                            }
                                ...
                                {
                                    CSSTagData* td = CSS_ALL_TAGS[door].data;
                                    if (td->use_tag == 0) {
                                        td->text->hidden = 1;
                                    }
                                }
                    ...
                    case 7: {
                        ...
                        updateGrabbedSlider(
                            cursor, &CSS_ALL_DOORS.doors[slider_door],
                ...
                if (mnCharSel_804D6CF5 == 1) {
                    ...
                    cpu_level_widget:
                        if (mnCharSel_804D6CB0->match_type <= 0xDU) {
                            f32 cy2 = cursor->x10;
                            if (cy2 < CSS_ALL_MISC.cpubtn_top &&
                                cy2 > CSS_ALL_MISC.cpubtn_btm)
                            {
                                f32 cx2 = cursor->xC;
                                if (cx2 > CSS_ALL_MISC.cpudown_left &&
                                    cx2 < CSS_ALL_MISC.cpudown_right)
                                {
                                    cursor->x8 = 1;
                                    if (trigger & HSD_PAD_A) {
                                        u8* level;
                                ...
                                } else if (cx2 > CSS_ALL_MISC.cpuup_left &&
                                           cx2 < CSS_ALL_MISC.cpuup_right)
                                {
                                    cursor->x8 = 1;
                                    if (trigger & HSD_PAD_A) {
                                        u8* level;
                ...
                try_costume_change: {
                    ...
                    if (mnCharSel_804A0BD0[cport2]->x5 == 0 &&
                        mnCharSel_804D6CB0->vs.start.rules.is_teams == 0 &&
                        CSS_ALL_DOORS.doors[cport2].p_kind != 3)
                    {
