...
void grBigBlue_801E6364(Ground_GObj* gobj)
{
    ...
    gp->u.bigblue.car.collision_jobjs =
        HSD_MemAlloc(sizeof(*gp->u.bigblue.car.collision_jobjs) * 30);
...
void grBigBlue_801E6C60(Ground_GObj* gobj)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        case 0: {
            if ((grBigBlue_801E89DC(0) != NULL) != 0) {
        ...
        case 2: {
            ...
            if (gp->u.bigblue.data[i].x4 <= 0) {
                ...
                if (right_y != -F32_MAX || left_y != -F32_MAX) {
                    ...
                    if ((grBigBlue_801E89DC(1) != NULL) != 0) {
                        gp->u.bigblue.data[i].x2 = 1;
                        pos.x = -(10.0f + Stage_GetBlastZoneRightOffset());
                    ...
                    if ((grBigBlue_801E89DC(2) != NULL) == 0) {
...
void grBigBlue_801E8D64(Ground_GObj* gobj)
{
    ...
    gp->u.bigblue.platform.xC8_timer = yakumono_param->xD8;
...
void grBigBlue_801E93D8(Ground_GObj* gobj)
{
    ...
    {
        s8 state = (s8) gp->u.bigblue.platform.xC4;
                    ...
                    ->u.bigblue.manager.event_data[1] != NULL)
            {
                ...
                if (count <= 1) {
                    ...
                    if (height != -3.4028235e38f) {
                        ...
                        gp->u.bigblue.platform.xD8 = yakumono_param->xD0;
                        HSD_JObjClearFlagsAll(jobj, JOBJ_HIDDEN);
                        gp->u.bigblue.platform.xC4 = 1;
                    }
                }
            }
            break;
        ...
        case 3:
            ...
            case 2: {
                ...
                if (timer <= 0) {
                    ...
                    gp->u.bigblue.platform.xD8 = yakumono_param->xD0;
                    gp->u.bigblue.platform.xC4 = 3;
                    Ground* manager = GET_GROUND(Ground_GetMapGObj(32));
                    idx = 0;

                    while (ctr != 0) {
                        if (manager->u.bigblue.data[idx].x1 == 0) {
                            manager->u.bigblue.data[idx].x1 = 2;
                            manager->u.bigblue.data[idx].x4 = 0;
                            break;
                        }
            ...
            }
            {
                ...
                {
                    f32 translate_x = gp->u.bigblue.platform.xD8;
        ...
        if (timer <= 0) {
            ...
            if (-3.4028235e38f != right_y || -3.4028235e38f != left_y) {
                ...
                if (collision == 0) {
                    ...
                    gp->u.bigblue.platform.target_y_dc = pos.y;
        ...
        if (ace_result == 0 || (ace_result == 1 && pos.y < y_check)) {
            if (bounds_y <= surface_y) {
                if (-3.4028235e38f == surface_y) {
                    gp->u.bigblue.platform.target_y_dc = half_top.y;
                } else {
                    gp->u.bigblue.platform.target_y_dc =
            ...
            gp->u.bigblue.platform.target_y_dc = pos.y + (pos.y - y_check);
        }

        {
            f32 diff = pos.y - gp->u.bigblue.platform.target_y_dc;
            if (diff < 0.0f) {
                diff = -diff;
            }
            if (diff < 0.5f) {
                vel_y = 0.0f;
            } else if (pos.y < gp->u.bigblue.platform.target_y_dc) {
                vel_y = (gp->u.bigblue.platform.target_y_dc - pos.y) /
                        yakumono_param->x124;
                                            ...
                                            s32* closest_lane)
{
    ...
    *closest_lane = -1;
    ...
    for (car_idx = 0; car_idx < 4; car_idx++) {
        struct grBigBlue_CarLane* car = &gp->u.bigblue.car.lanes[car_idx];
        u32 state = car->state;
...
void grBigBlue_801ECB50(Ground_GObj* gobj)
{
    ...
    for (i = 0; i < 4; i++) {
        u32 state = gp->u.bigblue.car.lanes[i].state;
        if (state != 1 && state != 7 && state != 8) {
            active_count++;
        }
    }
    ...
    /* Find closest car. */
    {
        ...
        if (found_ten == 0 && closest_lane != -1) {
            struct grBigBlue_CarLane* target_car =
                &gp->u.bigblue.car.lanes[closest_lane];
            target_car->state = 10;

            for (i = 0; i < 4; i++) {
                struct grBigBlue_CarLane* car = &gp->u.bigblue.car.lanes[i];
                u32 state = car->state;
                if ((state == 7 && car->pos.x < target_car->pos.x) ||
                    (state == 8 && car->pos.x > target_car->pos.x))
                {
                    car->state = 4;
                }
            }
...
void grBigBlue_801ED694(Ground_GObj* gobj, s32 lane)
{
    ...
    {
        u32 state = gp->u.bigblue.car.lanes[lane].state;
        f32 target;
    ...
    if (gp->u.bigblue.car.lanes[lane].direction) {
        ...
        if (-F32_MAX != ground_y && ground_y > rank_factor) {
            if (gp->u.bigblue.car.lanes[lane].angular_velocity < 0.0F) {
                gp->u.bigblue.car.lanes[lane].direction = 0;
...
heading_converge:
    ...
    if (-F32_MAX != ground_y && !(gp->u.bigblue.car.lanes[lane].direction)) {
...
bool grBigBlue_801EEF00(Ground_GObj* gobj, s32 index)
{
    ...
    case 5:
    case 6: {
        struct grBigBlue_CarLane* car = &gp->u.bigblue.car.lanes[index];
        f32* alpha = &car->alpha;
