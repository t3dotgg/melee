...
static void Measure(const char *name, void (*wait)(Clock::time_point), int fps)
{
  ...
  {
    const auto target = start + std::chrono::nanoseconds{1000000000LL * sample / fps};
    wait(target);
    const auto now = Clock::now();
    assert(now >= target);
    late.push_back(std::chrono::duration<double, std::micro>(now - target).count());
  }
  const double cpu_seconds = ThreadSeconds() - cpu_start;
  std::sort(late.begin(), late.end());
  std::printf("%s,%d,%d,%.3f,%.3f,%.3f,%.3f,%.3f\n", name, fps, samples, late[samples / 2],
              late[samples * 95 / 100], late[samples * 99 / 100], late.back(),
              cpu_seconds / 5 * 100);
}
...
int main()
{
  ...
  @autoreleasepool
  {
    ...
    std::puts("timer,fps,samples,p50_late_us,p95_late_us,p99_late_us,max_late_us,cpu_percent");
    for (int fps : {60, 120})
    {
